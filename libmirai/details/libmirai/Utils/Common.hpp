...
{
	using ReturnType = decltype(std::declval<typename nlohmann::json>().value(std::forward<KeyType>(key), std::forward<ValueType>(default_value)));
...
bool HasValue(const nlohmann::json& j, KeyType&& key)
{
	if (!j.is_object() || !j.contains(key) || j.at(key).is_null())
		return false;
	return true;
