...
{
	httplib::Params params = {{"sessionKey", SessionKey}, {"target", target.to_string()}, {"memberId", MemberId.to_string()}};
...
json HttpClientImpl::SendFriendMessage(const string& SessionKey, QQ_t qq, const json& message, std::optional<MessageId_t> QuoteId)
{
...
{
	httplib::Params params = {{"sessionKey", SessionKey}, {"target", target.to_string()}, {"withDownloadInfo", withDownloadInfo ? "true" : "false"}};
	if (!path.empty())
...
json HttpClientImpl::FileInfo(const string& SessionKey, const string& id, const string& path, UID_t target, bool withDownloadInfo)
{
	httplib::Params params = {{"sessionKey", SessionKey}, {"target", target.to_string()}, {"withDownloadInfo", withDownloadInfo ? "true" : "false"}};
	if (!path.empty())
		params.emplace("path", path);
...
json HttpClientImpl::FileMkdir(const string& SessionKey, const string& id, const string& path, UID_t target, const string& directory)
{
...
json HttpClientImpl::FileRename(const string& SessionKey, const string& id, const string& path, UID_t target, const string& name)
{
...
json HttpClientImpl::FileUpload(const string& SessionKey, const string& path, UID_t target, const string& type, const string& name, const string& content)
{
...
json HttpClientImpl::UploadAudio(const string& SessionKey, const string& type, const string& Audio)
{
	...
	{{"sessionKey", SessionKey, "", ""},
	 {"type", type, "", ""},
	 {"img", Audio, "", "application/octet-stream"}};
	this->_client.set_compress(true);
					...
					std::optional<bool> AutoApprove, std::optional<bool> AllowAnonymousChat)
{
	...
	if (!name.empty())
		body.at("config")["name"] = name;
	if (AllowConfessTalk.has_value())
		body.at("config")["confessTalk"] = AllowConfessTalk.value();
	if (AllowMemberInvite.has_value())
		body.at("config")["AllowMemberInvite"] = AllowMemberInvite.value();
	if (AutoApprove.has_value())
		body.at("config")["autoApprove"] = AutoApprove.value();
	if (AllowAnonymousChat.has_value())
		body.at("config")["anonymousChat"] = AllowAnonymousChat.value();
...
{
	httplib::Params params = {{"sessionKey", SessionKey}, {"target", target.to_string()}, {"memberId", member.to_string()}};
	auto result = this->_client.Get("/memberInfo", params, httplib::Headers{});
	json resp = Utils::ParseResponse(result);
	return resp;
}

json HttpClientImpl::PostMemberInfo(const string& SessionKey, GID_t target, QQ_t member, const string& name, const string& title)
{
	json body = {{"sessionKey", SessionKey}, {"target", target},
	{"info", json::object()}};
	if (!name.empty())
...
json HttpClientImpl::AnnoPublish(const string& SessionKey, GID_t target, const string& content, 
				const string& url, const string& path, const string& base64, 
				bool ToNewMember, bool pinned, bool ShowEditCard, bool ShowPopup, bool RequireConfirm)
{
	json body = {{"sessionKey", SessionKey}, {"target", target}, {"content", content},
	{"sendToNewMember", ToNewMember}, {"pinned", pinned}, {"showEditCard", ShowEditCard}, 
...
json HttpClientImpl::RespNewFriendRequestEvent(const string& SessionKey, int64_t EventId, QQ_t FromId, GID_t GroupId, int operate, const string& message)
{
...
json HttpClientImpl::RespMemberJoinRequestEvent(const string& SessionKey, int64_t EventId, QQ_t FromId, GID_t GroupId, int operate, const string& message)
{
	json body = {{"sessionKey", SessionKey}, {"eventId", EventId}, {"fromId", FromId},
	{"groupId", GroupId}, {"operate", operate}, {"message", message}};
...
json HttpClientImpl::RespBotInvitedJoinGroupRequestEvent(const string& SessionKey, int64_t EventId, QQ_t FromId, GID_t GroupId, int operate, const string& message)
{
	json body = {{"sessionKey", SessionKey}, {"eventId", EventId}, {"fromId", FromId},
	{"groupId", GroupId}, {"operate", operate}, {"message", message}};
...
json HttpClientImpl::CmdRegister(const string& SessionKey, const string& name, const std::vector<string>& alias, const string& usage,
